  ...
  const filteredLawyers = lawyers.filter((lawyer) => {
    ...
    if (Array.isArray(lawyer.specialization)) {
      return lawyer.specialization.some(
        (spec) => selectedSpecialties.includes(spec.specializationId)
      );
    }
    ...
    <div className="min-h-screen bg-gray-50 w-full">
      ...
      <div className="bg-gradient-to-r from-[#003366] via-[#004080] to-[#003366] text-white py-12 relative overflow-hidden px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 z-30">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-3 tracking-tight drop-shadow-lg">Өөрт тохирох хуульчийг сонгоорой</h1>
          <p className="text-sm sm:text-base md:text-lg lg:text-xl text-slate-200 font-medium drop-shadow-sm">Мэргэшсэн хуульчид таныг хүлээж байна</p>
        </div>
      </div>
                       ...
                       <div className="space-y-1 sm:space-y-2">
                         <div className="flex items-center justify-between">
                           <div className="flex items-center space-x-3">
                             ...
                             <h4 className="text-sm sm:text-base font-semibold text-gray-700 uppercase tracking-wide">Боломжтой ангилалууд</h4>
                           </div>
                           <span className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full font-medium">{specializations.length} ангилал</span>
                         </div>
                         <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3 lg:gap-4 overflow-visible relative z-30">
                           {specializations.map((spec: Category) => (
                             ...
                             >
                               ...
                               <span className="text-xs sm:text-sm text-gray-700 flex-1 leading-tight font-medium group-hover:text-blue-700 transition-colors duration-200 break-words hyphens-auto">{spec.categoryName}</span>
                             </label>
                           ))}
                         </div>
                       </div>
